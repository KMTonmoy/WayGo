...
const Page = () => {
  ...
  const handleDownload = ticket => {
    ...
    doc.text(`Name: ${ticket.passengerName}`, 10, 40);
    doc.text(`Gender: ${ticket.gender}`, 10, 50);
    doc.text(`Phone: ${ticket.passengerPhone}`, 10, 60);
    doc.text(`Email: ${ticket.email}`, 10, 70);
    doc.text(`Price: ${ticket.price} BDT`, 10, 80);
    doc.text(`From: ${ticket?.Bus?.from || 'N/A'}`, 10, 90);
    doc.text(`To: ${ticket?.Bus?.to || 'N/A'}`, 10, 100);
    doc.text(`Payment Date: ${ticket.paymentMonth}`, 10, 110);
    doc.text(`Departure Date: ${ticket.departureDate}`, 10, 120);
    doc.text(`Selected Seats: ${ticket.selectedSeats.join(', ')}`, 10, 130);
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 p-6">
      {userTickets.length > 0 ? (
        userTickets.map(ticket => (
          <motion.div
            key={ticket._id}
            className="bg-[#c14d70] text-white shadow-lg rounded-lg p-6 transition-transform transform hover:scale-105 relative flex flex-col justify-between"
          ...
          >
            <div className="flex-grow">
              ...
              <p className="mb-1">
                Name: <strong>{ticket.passengerName}</strong>
              </p>
              <p className="mb-1">
                Phone: <strong>{ticket.passengerPhone}</strong>
              </p>
              <p className="mb-1">
                Gender: <strong>{ticket.gender}</strong>
              </p>
              <p className="mb-1">
