  ...
  return (
    <div className="p-4 my-10">
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-[#F43F5E]">All Buses</h2>
        <button onClick={toggleLayout} aria-label="Toggle layout view">
          {layout === 'list' ? (
            <FaThLarge className="text-2xl text-[#F43F5E] hover:text-[#F43F5E] transition-colors" />
          ) : (
            <FaThList className="text-2xl text-[#F43F5E] hover:text-[#F43F5E] transition-colors" />
          )}
        </button>
      </div>
        ...
        <AnimatePresence>
          {busData.slice(0, visibleCount).map(bus => (
            ...
            >
              ...
              <div
                className={`${
                  layout === 'list' ? 'w-full md:w-2/3' : 'w-full text-left'
                }`}
              >
                {layout === 'list' ? (
                  <div>
                    <div className="flex justify-around py-5 items-center gap-4">
                      <div>
                        <h3 className="text-2xl font-semibold text-[#F43F5E] mb-2">
                          {bus.busName}
                        </h3>
                        <p>
                      ...
                      <Link href={`/AllBus/${bus._id}?date=${departureDate}`}>
                        <button className="px-6 py-2 bg-[#F43F5E] text-white rounded-lg hover:bg-[#F43F5E] transition-colors">
                          Book a Ticket
                        </button>
                      </Link>
                    </div>
                  ...
                  <div>
                    <h3 className="py-2 text-center text-2xl font-semibold text-[#F43F5E] mb-2">
                      {bus.busName}
                    </h3>
                    <div>
                      ...
                      <div className="flex flex-col items-center">
                        <p className="text-xl mb-5 font-bold text-[#F43F5E]">
                          Seat Price: {bus.seatPrice} BDT
                        </p>
                        <Link href={`/AllBus/${bus._id}?date=${departureDate}`}>
                          <button className="px-6 py-2 bg-[#F43F5E] text-white rounded-lg hover:bg-[#F43F5E] transition-colors">
      ...
      {busData.length > 6 && (
        <div className="text-center mt-6">
          <button
            className="px-8 py-3 bg-[#F43F5E] text-white rounded-full shadow-lg hover:bg-[#F43F5E] transition-all duration-300"
