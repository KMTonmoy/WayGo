...
const CustomSlider = () => {
  ...
  useEffect(() => {
    const intervalId = setInterval(nextSlider, 1000);
  ...
  return (
    <div className="relative mx-auto mt-8">
      <div className="relative overflow-hidden rounded-lg shadow-lg">
        <button
          onClick={prevSlider}
          className="absolute top-1/2 left-4 transform -translate-y-1/2 bg-orange-500 text-white rounded-full p-2 hover:bg-orange-600 transition duration-300"
        >
            ...
            >
              <div className="flex flex-col items-center">
                <img
                  ...
                  alt={deals[0].title}
                />
                <h6 className="text-2xl font-bold text-gray-800">{deals[0].title}</h6>
                <p className="text-gray-600 mt-2 text-center">{deals[0].description}</p>
                <p className="text-sm text-gray-500 mt-2">
                  Offer valid till: <span className="font-semibold">{deals[0].validTill}</span>
                </p>
                <div className="mt-4 bg-orange-100 text-orange-600 rounded-lg px-3 py-1 text-xs font-medium">
                  {deals[0].promoCode}
                </div>
